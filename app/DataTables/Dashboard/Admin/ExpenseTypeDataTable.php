...
class ExpenseTypeDataTable extends BaseDataTable
{
    public function __construct(DataTableRequest $request)
    {
        parent::__construct(new ExpenseType);
    ...
    public function dataTable($query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function (ExpenseType $record) {
                return view('dashboard.admin.salaries.expense-types.btn.actions', compact('record'));
            })
            ->editColumn('created_at', function (ExpenseType $record) {
                return $this->formatTranslatedDate($record->created_at);
            })
            ->editColumn('updated_at', function (ExpenseType $record) {
                return $this->formatTranslatedDate($record->updated_at);
            })
            ->addColumn('status', function (ExpenseType $record) {
                return $record->is_active
                    ? '<span class="badge bg-success">نشط</span>'
                    : '<span class="badge bg-danger">غير نشط</span>';
            })
            ->addColumn('responsible', function (ExpenseType $record) {
    ...
    public function query(): QueryBuilder
    {
        ...
        return ExpenseType::where('company_id', $user)->latest();
    }
