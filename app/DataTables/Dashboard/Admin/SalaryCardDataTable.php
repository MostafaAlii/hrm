...
class SalaryCardDataTable extends BaseDataTable
{
    public function __construct(DataTableRequest $request)
    {
        parent::__construct(new SalaryCard);
    ...
    public function dataTable($query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function (SalaryCard $record) {
                return view('dashboard.admin.salaryCards.btn.actions', compact('record'));
            })
            ->editColumn('created_at', function (SalaryCard $record) {
                return $this->formatTranslatedDate($record->created_at);
            })
            ->editColumn('updated_at', function (SalaryCard $record) {
                return $this->formatTranslatedDate($record->updated_at);
            })
            ->addColumn('responsible', function (SalaryCard $record) {
    ...
    public function query(): QueryBuilder
    {
        ...
        return SalaryCard::where('company_id', $user)->latest();
    }
