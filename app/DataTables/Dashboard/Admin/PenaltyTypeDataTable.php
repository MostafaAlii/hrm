...
class PenaltyTypeDataTable extends BaseDataTable
{
    public function __construct(DataTableRequest $request)
    {
        parent::__construct(new PenaltyType);
    ...
    public function dataTable($query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function (PenaltyType $record) {
                return view('dashboard.admin.penalty-types.btn.actions', compact('record'));
            })
            ->editColumn('created_at', function (PenaltyType $record) {
                return $this->formatTranslatedDate($record->created_at);
            })
            ->editColumn('updated_at', function (PenaltyType $record) {
                return $this->formatTranslatedDate($record->updated_at);
            })
            ->addColumn('responsible', function (PenaltyType $record) {
    ...
    public function query(): QueryBuilder
    {
        ...
        return PenaltyType::where('company_id', $user)->latest();
    }
