...
class SpecializationDataTable extends BaseDataTable
{
    ...
    public function dataTable($query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function (Specialization $record) {
                return view('dashboard.admin.specializations.btn.actions', compact('record'));
            })
            ->editColumn('created_at', function (Specialization $record) {
                return $this->formatTranslatedDate($record->created_at);
            })
            ->editColumn('updated_at', function (Specialization $record) {
                return $this->formatTranslatedDate($record->updated_at);
            })
            ->addColumn('responsible', function (Specialization $record) {
    ...
    public function query(): QueryBuilder
    {
        ...
        return Specialization::where('company_id', $user)->latest();
    }
