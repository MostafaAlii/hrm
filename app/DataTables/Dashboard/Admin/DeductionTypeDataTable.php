...
class DeductionTypeDataTable extends BaseDataTable
{
    public function __construct(DataTableRequest $request)
    {
        parent::__construct(new DeductionType);
    ...
    public function dataTable($query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function (DeductionType $record) {
                return view('dashboard.admin.salaries.deduction-types.btn.actions', compact('record'));
            })
            ->editColumn('created_at', function (DeductionType $record) {
                return $this->formatTranslatedDate($record->created_at);
            })
            ->editColumn('updated_at', function (DeductionType $record) {
                return $this->formatTranslatedDate($record->updated_at);
            })
            ->addColumn('status', function (DeductionType $record) {
                return $record->is_active
                    ? '<span class="badge bg-success">نشط</span>'
                    : '<span class="badge bg-danger">غير نشط</span>';
            })
            ->addColumn('responsible', function (DeductionType $record) {
    ...
    public function query(): QueryBuilder
    {
        ...
        return DeductionType::where('company_id', $user)->latest();
    }
