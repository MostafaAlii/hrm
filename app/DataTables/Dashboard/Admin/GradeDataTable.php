...
class GradeDataTable extends BaseDataTable
{
    ...
    public function dataTable($query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function (Grade $record) {
                return view('dashboard.admin.grades.btn.actions', compact('record'));
            })
            ->editColumn('created_at', function (Grade $record) {
                return $this->formatTranslatedDate($record->created_at);
            })
            ->editColumn('updated_at', function (Grade $record) {
                return $this->formatTranslatedDate($record->updated_at);
            })
            ->addColumn('responsible', function (Grade $record) {
    ...
    public function query(): QueryBuilder
    {
        ...
        return Grade::where('company_id', $user)->latest();
    }
